...
func SplitCookie(m mapC, cookie string) {
	splitCookie := strings.Split(cookie, "; ")
	for i := range splitCookie {
		keyelem := strings.Split(splitCookie[i], "=")
		m[keyelem[0]] = keyelem[1]
	}
